...
public class FruitGame : MonoBehaviour
{
    ...
    public float gameWicth = 6.0f;                                                 //게임판 너비
    ...
    void Update()
    {
        ...
        if (currentFruit != null)                     //현재 생성된 과일이 있을 때만 처리
        {
            ...
            Vector3 newPosition = currentFruit.transform.position;                           //과일 위치 없데이트

            newPosition.x = worldPosition.x;

            float halfFruitSize = fruitSize[currentFruitType] / 2f;

            if(newPosition.x < -gameWicth / 2 - halfFruitSize)
            {
                newPosition.x = -gameWicth / 2 - halfFruitSize;
            }
    ...
    void SpawnnewFruit()
    {
        if (!isGameOver)                         //게임 오버가 아닐 떄만 새 과일 생성
        {
            ...
            spawnPosition.x = Mathf.Clamp(spawnPosition.x, -gameWicth / 2 + halfFruitSize, gameWicth / 2 - halfFruitSize);
