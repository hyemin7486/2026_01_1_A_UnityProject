...
{
    public int gridWidht = 7;                 //가로 칸수
    public int gridHidht = 7;                 //세로 칸수
    ...
    {
        grid = new GridCell[gridWidht, gridHidht];                   //2차원 배열 생성

        for (int x = 0; x < gridWidht; x++)
        {
            for (int y = 0; y < gridHidht; y++)
            {
                Vector3 position = new Vector3(
                    x * CellSize - (gridWidht * CellSize / 2) + CellSize / 2,
                    y * CellSize - (gridHidht * CellSize / 2) + CellSize / 2,
                    1f
                );
    ...
    private GridCell FineEmptyCell()                    //비어 있는 칸 찾기
    {
        ...
        if (emptyCells.Count == 0)                                     //빈 칸이 없으면 null 값 변환
        {
            return null;
        }
