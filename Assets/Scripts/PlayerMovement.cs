...
public class PlayerMovement : MonoBehaviour
{
    ...
    [Header("지면 감지 설정")]
    public float coyotaTime = 0.15f;                                    //지면 관성 시간
    public float coyotaTimeCounter;                                     //관성 타이머
    ...
    public float glideTimerLeft;                                        //남은 사용 시간
    ...
    void Start()
    {
        ...
        glideTimerLeft = gliderMaxTime;                      //글라이더 시간 초기화
    }
    ...
    void Update()
    {
        ...
        if(Input.GetKey(KeyCode.G) && !isGrounded && glideTimerLeft > 0)    //G키를 누르면서 땅에 있지 않고 글라이더 남은 시간이 있을때 (3가지 조건)
        {
            ...
            glideTimerLeft -= Time.deltaTime;                        //글라이더 사용 시간 감소

            if(glideTimerLeft <= 0)                                  //사용 시간이 다 되면 비활성화
            {
                DisableGlider();
            }
        }
        ...
        if(isGliding)                                   //글라이딩 움직임 처리
        {
            ...
            AppiyGliderMovement(moveHorizontal, moveVertical);           //글라이더 사용 중 이동
        ...
        if (isGrounded)
        {
            ...
            glideTimerLeft = gliderMaxTime;                     //지상에 있을때 시간 회복
        }
    }
    ...
    void UpdateGrounededState()
    {
        ...
        else
        {
            ...
            if(coyotaTimeCounter > 0)
            {
                coyotaTimeCounter -= Time.deltaTime;                        //시간을 지속적으로 감소 시킨다.
            }
            else
            {
                isGrounded = false;
    ...
    void AppiyGliderMovement(float horizontal, float vertical)                //수평과 수직을 함수의 인수로 받는다.
    {
